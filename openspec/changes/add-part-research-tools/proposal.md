...
- **Unchanged contracts**: `check`/`verify` stay LLM-free and network-free (AC-2.1); research queries remain read-only; datasheet fetches write only the evidence cache before edit unlock; and the selection/evidence branches that write BOM.md or constraints.json require the same validated-change gate and obligations ledger as other design mutations.
